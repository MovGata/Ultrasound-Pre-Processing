...
namespace gui
{
    ...
    void Rectangle::dragStartEvent([[maybe_unused]] const SDL_Event &e)
    {
        ...
        for (const auto &r : subRectangles)
        {
            if (r->contains(mouse.x, mouse.y) && !r->process(e))
            {
                eventRectangle = r;
                SDL_Event ev;
                ...
                ev.user.data1 = &eventRectangle;
                ev.user.data2 = this;
                SDL_PushEvent(&ev);
                
                r->addCallback(SDL_MOUSEBUTTONUP, Rectangle::dragStopEvent);
                r->addCallback(SDL_MOUSEMOTION, Rectangle::dragEvent);
                
                break;
            }
        }
        
        // clearCallback(dropEventData);
    ...
    void Rectangle::dropEvent([[maybe_unused]] const SDL_Event &e)
    {
        ...
        {
            std::shared_ptr<Rectangle> rp = static_cast<std::weak_ptr<Rectangle> *>(e.user.data1)->lock();
        ...
        else if (e.user.code == DRAG_LINK)
        {
            ...
            for (auto &r : subRectangles)
            {
                ...
                if (r.get() != rp.get() && r->contains(mouse.x, mouse.y) && !r->process(e))
                {
                    ...
                    rp->outlinks.emplace_back(r, std::make_shared<glm::mat4>(linkArrow));
                    r->inlinks.emplace_back(rp, rp->outlinks.back().second);
                    break;
                }
            }
        }
    ...
    void Rectangle::moveStopEvent(const SDL_Event &e)
    {
        ...
        clearCallback(SDL_MOUSEBUTTONUP);
        clearCallback(SDL_MOUSEMOTION);
        // addCallback(SDL_MOUSEBUTTONDOWN, Rectangle::moveStartEvent);
    }
        ...
        for (const auto &r : subRectangles)
        {
            if (r->contains(mouse.x, mouse.y) && !r->process(e))
            {
                ...
                if (e.button.button == SDL_BUTTON_LEFT)
                {
                    ...
                    r->addCallback(SDL_MOUSEMOTION, Rectangle::moveEvent);
                    // SDL_PushEvent(&ev);
                }
                else if (e.button.button == SDL_BUTTON_RIGHT)
                {
                    ...
                    r->addCallback(SDL_MOUSEMOTION, Rectangle::updateArrow);
                }
                r->addCallback(SDL_MOUSEBUTTONUP, Rectangle::moveStopEvent);
                SDL_PushEvent(&ev);
                break;
            }
        }

        // clearCallback(moveEventData);
        // clearCallback(dropEventData);
    }

    void Rectangle::stopEvent(const SDL_Event &e)
    {
        std::shared_ptr<Rectangle> rec = static_cast<std::weak_ptr<Rectangle> *>(e.user.data1)->lock();
        bool child = false;
        for (auto &r : subRectangles)
        {
            if (r.get() == rec.get())
    ...
    void Rectangle::volumeStartEvent([[maybe_unused]] const SDL_Event &e)
    {
        ...
        for (const auto &r : subRectangles)
        {
            if (r->contains(mouse.x, mouse.y) && !r->process(e))
            {
                ...
                if (e.button.button == SDL_BUTTON_LEFT)
                {
                    ...
                    r->volume->addCallback(SDL_MOUSEMOTION, Volume::rotateEvent);
                }
                else if (e.button.button == SDL_BUTTON_RIGHT)
                {
                    ev.user.code = VOLUME_DRAG;
                    r->volume->addCallback(SDL_MOUSEMOTION, Volume::dragEvent);
                }

                r->addCallback(SDL_MOUSEBUTTONUP, Rectangle::volumeStopEvent);
                r->addCallback(SDL_MOUSEMOTION, Rectangle::volumeBypass);
        ...
        // clearCallback(volumeEventData);
        // addCallback(volumeEventData, Rectangle::doubleDown);
    }
    ...
    void Rectangle::doubleDown([[maybe_unused]] const SDL_Event &e)
    {
        ...
        if (ev.user.code == VOLUME_ROTATE)
        {
            ev.user.code = VOLUME_DRAG;
            volume->addCallback(SDL_MOUSEMOTION, Volume::rotateEvent);
        }
        else if (ev.user.code == VOLUME_DRAG)
        {
            ev.user.code = VOLUME_ROTATE;
